...
/** Base class for all calendar servlet methods.
 ...
 */
public abstract class CalendarMethodBase extends MethodBase {
  private ObjectMapper objectMapper;
  private ConfigCommon config;
  private BwWebGlobals webGlobals;
  ...
  public boolean beforeMethod(final HttpServletRequest req,
    ...
    webGlobals = (BwWebGlobals)rutil.getSessionAttr(BwWebGlobals.webGlobalsAttrName);
    if (webGlobals == null) {
      if (debug()) {
        debug("No WebGlobals attribute found in request - creating");
      }
      webGlobals = newWebGlobals();
      rutil.setSessionAttr(BwWebGlobals.webGlobalsAttrName,
                           webGlobals);
    } else if (debug()) {
   ...
   */
  public BwWebGlobals newWebGlobals() {
    return new BwWebGlobals();
  }

  public BwWebGlobals getWebGlobals() {
    return webGlobals;
  }
   ...
   */
  public void init(final ConfigCommon config,
